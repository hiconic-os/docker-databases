...
__waitTillServerStarts() {
	...
	for (( i=0; i<30 ; i++ )) ; do
		...
		# If it ever fails, use "docker logs hc-test-mssql" to see what other outputs from the server come later (the output from docker logs is the content of the errorlog file below).
